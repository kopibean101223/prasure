...
try {// Load the MySQL driver
    ...
    int h=510;
    JLabel brb = new JLabel("BORROWED BOOKS:    ");
    		brb.setBounds(200, 460,1000, 60);
    brb.setFont(fontotpt);
    brb.setForeground(Color.BLACK);
    brb.setHorizontalAlignment(SwingConstants.LEFT);
    		f.add(brb);
    ...
    while (resultSet2.next()) {
    	JLabel RSlbl = new JLabel((resultSet2.getString(1)));
    	f.add(RSlbl);
    	RSlbl.setBounds(340, h,1000, 60);
    	RSlbl.setFont(fontotpt);
    	RSlbl.setForeground(Color.BLACK);
    	RSlbl.setHorizontalAlignment(SwingConstants.LEFT);
    	h+=40;
